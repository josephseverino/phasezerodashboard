...
var express = require('express'),
    ...
    test = require('assert');
var arrayData = [{a:'this'},{a:'that'}];
var url = 'mongodb://localhost/ACLED';
MongoClient.connect(url, function(err, db) {
    // Get the collection
    var col = db.collection('Nigeria');
    col.insertMany(arrayData, function(err, r) {
        test.equal(null, err);
...
app.use(sessions({
    ...
    duration: 86400 * 1000, // 60 * 60 * 24 (number of seconds in a day), tells the middleware when the cookie/session should expire,
    cookie: {
